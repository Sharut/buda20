...
#########################################################
#meta
r = open('NLP/PAN20/data/raw/es/truth.txt', "r")
data = r.read().split("\n")
idk = [] #id
spreader = [] #yes or no
...
pathlist = Path('NLP/PAN20/data/raw/es').glob('**/*.xml')
ids=[]
x_raw=[]
s=0
...
es_data.to_csv('NLP/PAN20/data/es_data_tweetenkent.tsv', sep='\t', index=False)
...
es_data_tweet_consist = pd.DataFrame(list(zip([es_data["ID"][i*100] for i in range(int(len(es_data["ID"])/100))],
                                              [es_data["spreader"][i*100] for i in range(int(len(es_data["ID"])/100))])),
                                                                columns =['ID', "spreader"])
...
es_data_tweet_consist.to_pickle('NLP/PAN20/data/es_data_tweet_consist.pkl')